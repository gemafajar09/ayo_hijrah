<?php
  ...
  switch ($aksi){
    case "hapuscustomer" :
    if(isset($_GET['id'])){
      mysqli_query($con, "DELETE FROM tbl_customer where id_customer='$_GET[id]'");
...
<?php
    ...
    case "editcustomer":
      if (isset($_GET['id'])) {
        $sql = mysqli_query($con, "SELECT * FROM tbl_customer where id_customer='$_GET[id]'");
        $data = mysqli_fetch_assoc($sql);
      }
      if (isset($_POST['save'])) {
        if (!empty($_POST['password'])) {
          $password = $_POST['password'];
          $save = mysqli_query($con, "UPDATE tbl_customer set email='$_POST[email]', password='$password' where  id_customer='$_GET[id]'");
        } else if (empty($_POST['password'])) {
          $save = mysqli_query($con, "UPDATE tbl_customer set email='$_POST[email]'  where  id_customer='$_GET[id]'");
        }
        if ($save) {
          echo "<script>
            ...
            <div class="box-body">
              <div class="table table-responsive">
                <table id="example1" class="table table-bordered table-striped">
                  ...
                  <tbody>
              			<?php
              				$q=mysqli_query($con, "SELECT * from tbl_customer");
              				$no=1;
...
if(isset($_POST['simpanSet']))
{
  ...
  $update = mysqli_query($con,"UPDATE tbl_customer set jenis_toko='$jenis_toko'  where  id_customer='$id'");
  if ($update) {
          echo "<script>
                 window.location='?page=customer';
                </script>";
